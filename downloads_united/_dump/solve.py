...
tries = 0
for guess in itertools.product(keys, repeat=10):
    cracker = flag
    for key in list(guess):
        cracker = xor(cracker, bytearray(key.to_bytes(4, 'big')))
        try:
            cracker = bytearray(b64decode(cracker))
        except:
            continue
    try:
        resp = cracker.decode('ascii')
        print(resp)
    except:
        continue
